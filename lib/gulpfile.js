...
const config = {
  libraryName: 'angular-auth-oidc-client',
  unscopedLibraryName: 'angular-auth-oidc-client',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('clean:dist', () => {
  return del(
    [
      '!'+config.outputDir+'.git',
      '!'+config.outputDir+'lib',
      config.buildDir, 
      config.outputDir+'module', 
      config.outputDir+'esm2015', 
      config.outputDir+'esm5', 
      config.outputDir+'bundles', 
      config.outputDir+'API_DOCUMENTATION.md', 
      config.outputDir+'CHANGELOG.md', 
      config.outputDir+'index.d.ts', 
      config.outputDir+'LICENSE', 
      config.outputDir+'package.json', 
      config.outputDir+'qpons-module.d.ts', 
      config.outputDir+'qpons-module.metadata.json', 
      config.outputDir+'README.md',
      config.outputDir+config.unscopedLibraryName+'.d.ts',
      config.outputDir+config.unscopedLibraryName+'.metadata.json'
    ], {force: true}
  );
});
...
gulp.task('build', ['clean'], (cb) => {
  runSequence('compile', 'test', 'npm-package', 'rollup-bundle', 'clean:build', 'build:doc', cb);
});
...
gulp.task('npm-package', (cb) => {
  ...
  targetPkgJson['main'] = `./bundles/${config.unscopedLibraryName}.umd.min.js`;
  ...
  pump(
    [
      gulp.src(['README.md', 'LICENSE', 'CHANGELOG.md', 'API_DOCUMENTATION.md',
...
gulp.task('rollup-bundle', (cb) => {
  ...
  .then(() => {
    ...
    const globals = {
      ...
      'rxjs/add/observable/interval': 'Rx.Observable.prototype',
